...
This repository implements **Persistent Nonnegative Matrix Factorization (pNMF)** for learning multi-scale low-rank embeddings. 
The project includes experiments on both simulation datasets and single-cell RNA-seq datasets.
...
You can install the required packages using `pip` from the `requirements.txt` file.
...
-   For **simulation datasets** (e.g., `3D_concentric_circles`), we recommend:

``` bash
--l1 100 --l2 100 --l3 1
```

-   For **single-cell datasets** (e.g., `GSE75748time`, `Dendritic_batch1`), we recommend:
